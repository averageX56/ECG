...
Для A100/JupyterLab используйте отдельный [cluster setup](docs/CLUSTER.md):
`bash scripts/setup_cluster.sh`, затем kernel **ECG A100 (Python 3.11)**.
Существующие datasets/artifacts сохраняются на своих местах.

На кластере используйте setup выше, откройте notebook и проверьте RUN-флаги: по умолчанию выбран Qwen E/1.7B с curriculum, остальные ветки выключены. Необходимые кэши проверяются до запуска. Предобработка и инженерные features поддерживаются; train/valid/test разделены по доступным идентичностям пациентов и источникам.
