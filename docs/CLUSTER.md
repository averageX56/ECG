...
Проверка перед запуском: `python scripts/preflight.py --profile smoke`; на A100 — `--profile bert` или `--profile founder`. Скрипт только читает файлы и сообщает о недостающих данных/GPU.
