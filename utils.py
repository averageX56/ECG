...
# ВАЖНО: реальные файлы называются 'all_feats_ch1.zip' (без подчёркивания перед номером
# канала), а не 'all_feats_ch_1.zip'. Старый паттерн '*/*all_feats_ch_{ch_idx}.zip' никогда
# не совпадал с ними, поэтому glob() возвращал пустой список и pd.concat([]) падал с
# ValueError: No objects to concatenate. Ищем оба варианта имени, рекурсивно, чтобы не
# зависеть от глубины вложенности подпапок датасетов (CPSC-Extra/Georgia/PTB/...).
_feats_cache_files = sorted(set(
    list(Path('feats/').rglob(f'*all_feats_ch{ch_idx}.zip')) +
    list(Path('feats/').rglob(f'*all_feats_ch_{ch_idx}.zip'))
))
if not _feats_cache_files:
    raise FileNotFoundError(
        f"Не найдено ни одного файла '*all_feats_ch{ch_idx}.zip' под 'feats/'. "
        "Проверьте, что рабочая директория указывает на корень проекта и что "
        "предвычисленные фичи действительно лежат в подпапках feats/<Dataset>/."
    )
all_feats = pd.concat([pd.read_csv(f, index_col=0) for f in _feats_cache_files], ignore_index=True)
