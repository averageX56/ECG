...
def audit(root=".", output="artifacts/catalog.csv"):
    ...
    for directory in tqdm(directories, desc="Datasets", unit="dataset"):
        ...
        for p in tqdm(
            ...
            if source == "LUDB":
                patient = "LUDB:" + p.stem
                identity = "dataset_unique_subject"
            ...
            if source == "WFDB_PTB-XL" and ptb is not None:
                ...
                if idx in ptb.index:
                    ...
                    patient = "PTBXL:" + str(int(m.patient_id))
                    fold = int(m.strat_fold)
                    identity = "official_patient_id"
            ...
            row = dict(
                ...
                n_leads=h["n_leads"],
                duration_s=h["n_samples"] / h["fs"],
                labels=";".join(codes),
                has_labels=bool(codes),
                patient_id=patient,
                identity=identity,
                fold=fold,
            ...
            row.update(
                {
                    k: int(bool(set(codes) & set(v))) if codes else -1
            ...
            rows.append(row)
            counts["records"] += 1
            counts["labeled"] += bool(codes)
            counts["readable"] += data.exists()
            dx.update(codes)

        if counts:
            summaries[directory.name] = {
                "counts": dict(counts),
                "diagnoses": dict(dx),
            }
    ...
    save_json(
        ...
        dict(
            ...
            notes=[
                "Unknown patient identities are not claimed to be patient-disjoint.",
    ...
    print(
        frame.groupby("source")[["readable", "has_labels"]]
        .agg(["count", "sum"])
        .to_string(),
        flush=True,
    )
