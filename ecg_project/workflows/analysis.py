...
def analyze(path,output='reports/example',checkpoint=None,csv_fs=None,
            ...
            hubert_run=None,hubert_model_root='artifacts/hubert_large',record_model_path=None,vt_model_path=None):
    ...
    vt_beats=beat_results
    if vt_model_path:
        from ecg_project.training.beats import predict
        vt_beats=predict(xx,peaks,predicted[ch],rec.fs,vt_model_path,checkpoint=checkpoint)
    episodes=ventricular_runs(vt_beats,rec.fs)
    for ep in episodes:
    ...
    result=dict(record_id=rec.record_id,fs=rec.fs,leads=rec.leads,unit=rec.unit,metadata=rec.metadata,
        ...
        provenance=dict(delineator=str(checkpoint),record_model=str(model) if model else None,beat_model=str(beat_model) if beat_model else None,
                        vt_candidate_model=str(vt_model_path or beat_model) if vt_model_path or beat_model else None))
    if hubert_run:
